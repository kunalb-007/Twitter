  ...
  return (
    <div className='xl:w-[600px] hidden lg:inline ml-8 space-y-5'>
        ...
        <div className='sticky top-16 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]'>
            ...
            {randomUsers.slice(0, randomUserNum).map((randomUsr) => (
                <div key={randomUsr.login.username} className="flex items-center px-4 py-2 cursor-pointer hover:bg-gray-200">
                    <img className='rounded-full' width={40} src={randomUsr.picture.thumbnail} alt='' />
                    <div className='truncate ml-4 leading-5'>
                        <h4 className='font-bold hover:underline text-[13px] truncate'>{randomUsr.login.username}</h4>
                        <h5 className='text-[12px] text-gray-500 truncate'>{randomUsr.name.first + " " + randomUsr.name.last}</h5>
                    </div>
